...
describe('editFrontmatter — nested values', () => {
  ...
  // Only the leaves that changed are written, so everything else in the block
  // keeps the shape its author gave it.
  test('editing a nested leaf keeps comments in that map', () => {
    const src =
      '---\ntitle: keep\nmeta:\n  # owner is load-bearing\n  status: open\n  owner: ada\n---\nbody\n';
    const { content, outcome } = editFrontmatter(src, (fm) => {
      (fm.meta as Record<string, unknown>).status = 'done';
    });

    expect(outcome).toBe('edited');
    expect(content).toContain('# owner is load-bearing');
    expect(content).toContain('status: done');
    expect(content).toContain('owner: ada');
  });

  test('an untouched nested block scalar keeps its | style', () => {
    const src =
      '---\nmeta:\n  note: |\n    line1\n    line2\nstatus: open\n---\nbody\n';
    const { content } = editFrontmatter(src, (fm) => {
      fm.status = 'done';
    });
  ...
  // A block scalar cannot carry a value ending in more than one newline, so a
  // multi-line string this function writes is quoted — but only the node it
  // writes, which is why the case above survives.
  test('a multi-line value written here round-trips exactly', () => {
    for (const value of ['a\n\n', 'line1\nline2\n', 'x\n\n\n']) {
      const { content } = editFrontmatter('---\nt: 1\n---\nbody\n', (fm) => {
        fm.s = value;
      });

      expect(parseFrontmatter(content).frontmatter.s).toBe(value);
    }
  });

  // Rebuilding a map from its own fields reorders keys without changing
  // anything. Reporting that as an edit makes a normalisation script rewrite
  // every note in the vault on every run.
  test('a nested map rebuilt in a different key order is unchanged', () => {
    const src = '---\nmeta:\n  owner: ann\n  status: draft\n---\nbody\n';
    const { content, outcome } = editFrontmatter(src, (fm) => {
      const meta = fm.meta as Record<string, unknown>;
      fm.meta = { status: meta.status, owner: meta.owner };
    });
  ...
  test('a reordered array IS a change', () => {

...
- **Navigation** (top nav, sidebars, outline) is configured in
  `website/.vitepress/config.ts`; the guide sidebar groups are shared with the
  API sidebar from one `guideSidebar` const there.
- **Theme overrides** live in `website/.vitepress/theme/` — the default theme
  plus a short `style.css` (centred image-less hero, non-colour-only active nav
  states). Keep it minimal; prefer theme config over CSS.
